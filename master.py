...
class MRenderJob(netrender.model.RenderJob):
    ...
    def addFrame(self, frame_number, command):
        frame = MRenderFrame(frame_number, command, self.output_file_ext)
...
class MRenderFrame(netrender.model.RenderFrame):
    def __init__(self, frame, command, output_file_ext):
        super().__init__()
        self.number = frame
        self.slave = None
        self.time = 0
    ...
    def getRenderFilename(self):
        return "%06d." % self.number + self.output_file_ext
...
render_pattern = re.compile("/render_([a-zA-Z0-9]+)_([0-9]+).([a-zA-Z]{3,4})")
